...
module.exports = app => {
    ...
    app.post('/login', async (req, res) => {
        try {
            let user = await UsersModel.findOne({
                ...
                room: {$regex: _.escapeRegExp(req.body.room), $options: "i"}
            }).lean().exec();
            ...
            } else res.status(400).send({message: "User not exist or password and room not correct"});
        } catch (e) {
    ...
    app.post('/register', async (req, res) => {
        try {
            ...
            const token = createToken({id: user._id, username: user.username, room: user.room});
